...
export default function Page() {
  ...
  async function start() {
    ...
    const ws = new WebSocket(
      `ws://${location.hostname}:8787/realtime?provider=deepgram`,
    );
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (msg.type === "partial") {
          setLines((prev) => {
            const copy = [...prev];
            if (partialIndexRef.current == null) {
              copy.push(msg.text);
              partialIndexRef.current = copy.length - 1;
            } else {
              copy[partialIndexRef.current] = msg.text;
            }
            return copy;
          });
  ...
  return (
    <div
      dir="rtl"
      style={{ padding: 20, display: "grid", gap: 12, maxWidth: 800 }}
    >
