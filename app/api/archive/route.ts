...
function safeName(s: string): string {
  return (s || 'report').replace(/[^\p{L}\p{N}\s._-]+/gu, '').trim() || 'report';
}
...
export async function GET(req: Request) {
  ...
  if (download) {
    ...
    const mime =
      download.toLowerCase().endsWith('.docx')
...
export async function POST(req: Request) {
  try {
    ...
    const ts = new Date().toISOString().replace(/[-:]/g, '').replace(/\..+/, ''); // 20251022T123456
    ...
    await fs.writeFile(path.join(ARCHIVE_DIR, jsonName), JSON.stringify(meta, null, 2), 'utf8');
