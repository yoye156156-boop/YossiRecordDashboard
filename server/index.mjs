...
wss.on("connection", (ws) => {
  ...
  const dgUrl =
    "wss://api.deepgram.com/v1/listen?encoding=linear16&sample_rate=16000&channels=1&language=he&interim_results=true&smart_format=true";
  ...
  ws.on("close", () => {
    ...
    out.end(() => {
      try {
        ...
        p.writeUInt32LE(36 + bytes, 0);
        fs.writeSync(fd, p, 0, 4, 4); // RIFF size
        p.writeUInt32LE(bytes, 0);
        fs.writeSync(fd, p, 0, 4, 40); // data size
        fs.closeSync(fd);
      } catch (e) {
        console.error("WAV header patch error:", e);
      }
