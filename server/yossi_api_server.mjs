...
/* --- recordings route (recursive) --- */
import * as fss from "node:fs"; // שימוש בסנכרוני כדי לפשט את החיפוש
const RECORDINGS_ROOT = path.resolve(process.cwd(), "recordings");
...
app.get("/recordings", (_req, res) => {
  try {
    ...
    function walk(dir) {
      ...
      for (const e of fss.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (e.isDirectory()) {
          ...
          out.push({
            name: e.name,
            rel: path.relative(RECORDINGS_ROOT, full),
            full,
            size: st.size,
            mtime: st.mtimeMs,
          });
    ...
    walk(RECORDINGS_ROOT);
    res.json({ root: RECORDINGS_ROOT, count: out.length, files: out });
  } catch (err) {
