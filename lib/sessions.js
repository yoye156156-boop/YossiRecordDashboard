...
export function importSessions(payload) {
  ...
  for (const raw of incoming) {
    ...
    const at = typeof raw.at === 'string' && !Number.isNaN(Date.parse(raw.at))
      ? raw.at
      : new Date().toISOString();
    const lines = Array.isArray(raw.lines) ? raw.lines.filter(x => typeof x === 'string') : [];
    byId.set(id, { id, at, lines });
  }
  const merged = Array.from(byId.values()).sort((a, b) => (Date.parse(b.at) || 0) - (Date.parse(a.at) || 0));
  writeAll(merged);
